...
// for API disqus more 100 - https://disqus.com/api/docs/cursors/
...
<div class="item-page">
  ...
  echo "<div class='righappuse'><a style='color:#000' href='https://disqus.com/api/applications/{$disqusApplication}/usage/' target='_blank'>API: {$valuusageapp}</a> / <a style='color:#000; text-decoration:underline;' href='https://embed.disqus.com/' target='_blank'>Встроить комментарий</a></div>";
...
if (JComponentHelper::isEnabled('com_zoo') == '1') {
  echo '<input type="checkbox" name="checkbadcomments" value="yes" id="togglecalend"> <label for="togglecalend" class="checkbox"> <b>Только плохие комментарии (диапазон)</b> </label>';
...
if ($lpdisqusidcommentapprove) {
  $approveAr = $httpClient->request('https://disqus.com/api/3.0/posts/approve.json?', [
    'api_key' => $disqusApiPublic,
    'access_token' => $disqusApiToken,
...
if ($updateAr->code == 200) {
          ...
          foreach ($listPostsres as $listPostsrespost) {
            ...
            if (!empty($lpdisqusraw_message)) {
              ...
              if ($lpdisqusprofileUrl && $lpdisqususername && $lpdisqusisAnonymous == 'Нет') {
                ...
                <input class='btn btn-mini' type='submit' value='{$lpdisqusname} /  {$lpdisqususername}' > </form> <a title='Перейти в профиль Disqus' href='{$lpdisqusprofileUrl}' target='_blank'><em class='icon-out-2'></em> </a> ";     if ($lpdisqusabout) {   echo "<span class='osebe'><b>О себе:</b> {$lpdisqusabout}</span></li>";   }
              }
              else {
              ...
              <input class='btn btn-mini btn-danger' type='submit' value='Удалить' > </form>


              <form  class='flform' action='/administrator/index.php?option=com_myjbzoostat&view=disqus' method='post' >
              <input type='hidden' name='userdisqusform' placeholder='Введите url' value='{$urllistPosts}'>
              <input type='hidden' name='lpdisqusidcommentspam'  value='{$lpdisqusidcomment}' />
              <input class='btn btn-mini btn-inverse' type='submit' value='Спам' > </form>


              <form  class='flform' action='/administrator/index.php?option=com_myjbzoostat&view=disqus' method='post' >
              <input type='hidden' name='userdisqusform' placeholder='Введите url' value='{$urllistPosts}'>
              <input type='hidden' name='lpdisqusidcommentreport'  value='{$lpdisqusidcomment}' />
              <input class='btn btn-mini btn-warning' type='submit' value='Предупреждение' > </form>

              <form  class='flform' action='/administrator/index.php?option=com_myjbzoostat&view=disqus' method='post' >
              <input type='hidden' name='userdisqusform' placeholder='Введите url' value='{$urllistPosts}'>
              <input type='hidden' name='lpdisqusidcommentapprove'  value='{$lpdisqusidcomment}' />
              <input class='btn btn-mini btn-success' type='submit' value='Опубликовать' > </form>
  ...
  <?php
  	if (JComponentHelper::isEnabled('com_zoo') == '1') {
  if (empty($urllistPosts) && empty($userdisqusform) && empty($checkbadcomments)) {
...
$date1 = date('Y-m-d',strtotime($calendstart));
$date2 = date('Y-m-d',strtotime($calendend));
...
->select($db->quoteName('id'))
->from($db->quoteName(ZOO_TABLE_ITEM))
->where($db->quoteName('created') . ' BETWEEN "' .$date1.' 00:00:01' . '" AND "' .$date2.' 23:59:59"');

$db->setQuery($querys);

$itemIdsResultsReport = $db->loadObjectList();

$numlistPostsrespost = '0';

foreach ($itemIdsResultsReport as $valueitem => $keyvalid) {

  $itemnothavatags = $app->table->item->get($keyvalid->id);

  $genurlbad = JRoute::_($app->jbrouter->externalItem($itemnothavatags, false), false, 2);
  // HACK:
  // $genurlbad = str_replace('local','com',$genurlbad);
  // END HACK:
        ...
        $mylistdisalldata = $disqusflagged;

        $datalistPostsbad = [
          'forum' => $disqusApiShort,
          'thread' => 'link:'.$genurlbad,
          'api_key' => $disqusApiPublic,
          // 'order'   => 'asc',
          'order'   => 'desc',
          'limit'   => '100',
        ...
        // $url = 'https://disqus.com/api/3.0/threads/listPosts.json?'. http_build_query($datalistPosts,null,'&').'&'.$mylistdisalldata;
        $urlbad = 'https://disqus.com/api/3.0/posts/list.json?'. http_build_query($datalistPostsbad,null,'&').'&'.$mylistdisalldata;

        if (!empty($lpdisqusidcommentedit) && !empty($lpdisqusidcomment)) {

          echo "<div class='editcommentplace'>
                        <form  class='flform' action='/administrator/index.php?option=com_myjbzoostat&view=disqus' method='post' >
                        <input type='hidden' name='userdisqusform' placeholder='Введите url' value='{$genurlbad}'>
                        <input type='hidden' name='lpdisqusidcomment' value='{$lpdisqusidcomment}'>
                        <textarea  type='text' name='lpdisqusidcommentupdate'>{$lpdisqusidcommentedit}</textarea>
                        <input class='btn  btn-primary lpdisqusidcommentupdate' type='submit' value='Обновить комментарий' > </form>";
        ...
        echo "<div style='width: 98%' class='scrollbar-inner hideedit'>";

        if (!empty($genurlbad) && empty($lpdisqusidcommentedit)) {

          $responcebad = $httpClientj->get($urlbad, null , null);
          $responcebad = $responcebad->body;
          $listPostsbad = json_decode($responcebad, true);

          if ($listPostsbad['code'] == 13) {
            echo "<p class='bg-danger' align='center'><small>Превышен лимит Disqus, необходимо немного подождать.</small></p>";
          }
          else {
            $listPostsresbad = $listPostsbad['response'];
            // asort($listPostsresbad);
            // dump($listPostsresbad,0,'$listPostsresbad');

            foreach ($listPostsresbad as $listPostsrespost) {
              //dump($listPostsrespost,0,'$listPostsrespost');
