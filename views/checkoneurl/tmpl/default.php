...

{
    ...
    echo '<form action="/administrator/index.php?option=com_myjbzoostat&view=checkoneurl" method="post" class="form-inline">';
    ...
    echo '<input id="myurltosite" type="text" name="myurltosite" placeholder="Введите полный URL">';
    echo '</div>';
    echo '<input type="submit"  class="btn" value="Поиск"></form>';
    echo '</div>';

  if (!empty($yayear) && !empty($yamonth) && empty($calendstart) && empty($calendend) && empty($myurltosite)) {
...
if($myurltosite):
    ...
    $ApiYandexURLV2 .= '&filters=(ym:pv:URL=*%27'.$myurltosite.'%27)';
...
//сводка трафика
    ...
    $ApiYandexTrafficSource .= '&filters=(ym:pv:URL=*%27'.$myurltosite.'%27)';
...
//результаты поиска
    ...
    $ApiYandexSearchEngine .= '&filters=(ym:pv:URL=*%27'.$myurltosite.'%27)';
...
//с социальных сетей
    ...
    $ApiYandexSocialNetwork .= '&filters=(ym:pv:URL=*%27'.$myurltosite.'%27)';
...
//переходы с сайтов
    ...
    $ApiYandexexternalReferer .= '&filters=(ym:pv:URL=*%27'.$myurltosite.'%27)';
...
// dump($DataResponceApiYandexexternalReferer,0,'DataResponceApiYandexexternalReferer');
    ...
    $ApiYandexisRobot .= '&filters=(ym:pv:URL=*%27'.$myurltosite.'%27)';
...
<?php  if (!empty($countDaysUrl)) : ?>

<div class="uk-grid uk-container-center uk-text-center">
  <div class="uk-panel uk-panel-box uk-panel-box-primary uk-width-1-1 nomargin">
...
<div class="uk-grid uk-container-center uk-text-center">
  ...
  <?php if (!empty($disqusApiShort)) : ?>
  <div class="uk-width-1-5"><div class="uk-panel uk-panel-box uk-panel-box-secondary">
  <h3 class="uk-panel-title uk-text-warning">Комментариев </h3>
  <div class="uk-text-large"><big><big><span class='disqus-comment-count' data-disqus-url='<?php echo $myurltosite; ?>'</span></big></big></div>
  </div></div>
  <?php endif; ?>
  <div class="uk-width-1-5"><div class="uk-panel uk-panel-box uk-panel-box-secondary">
  <h3 class="uk-panel-title uk-text-danger">Популярное </h3>
  <div class='ya-share2' data-services='vkontakte,facebook,odnoklassniki,moimir,gplus' data-access-token:facebook='<?php echo $fb_app_token; ?>'  data-url='<?php echo $myurltosite; ?>' data-size='m' data-counter=''></div>
 </div></div>
</div>
...
echo ' <div class="uk-width-1-6"><div class="uk-panel uk-panel-box uk-panel-box-secondary">
  <h4 class="uk-panel-title uk-text-muted minismall">'.$SocialNetworkName.'</h4>';
